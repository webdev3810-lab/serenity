...
export function SupabaseAdminDashboardV2({ email, role }: { email: string; role: AdminRole }) {
  ...
  const load = async () => {
    ...
    setError("");
    const [propertyResult, imageResult, amenityResult, reviewResult, datePriceResult, bookingResult, calendarResult, enquiryResult, homepageResult, settingsResult] = await Promise.all([
      ...
      supabase.from("bookings").select("*").order("created_at", { ascending: false }).limit(250),
      supabase.from("calendar_events").select("*").order("start_date", { ascending: true }).limit(500),
    ...
    const firstError = [propertyResult, imageResult, amenityResult, reviewResult, datePriceResult, bookingResult, calendarResult, enquiryResult, settingsResult].find((result) => result.error)?.error;
