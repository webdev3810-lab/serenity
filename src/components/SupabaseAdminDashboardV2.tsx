...
type Tab = AdminTab;
type HomepageSectionKey = "hero" | "search" | "featured" | "benefits" | "corporate" | "location" | "faqs" | "cta";
...
function HomepageSectionsEditor({ homepage, setHomepage, published, saveSection, saving, properties, toggleFeatured }: { homepage: HomepageDraft; setHomepage: (value: HomepageDraft) => void; published: boolean; saveSection: (section: HomepageSectionKey, published: boolean) => Promise<void>; saving: boolean; properties: Row[]; toggleFeatured: (property: Row) => Promise<void> }) {
  const [activeSection, setActiveSection] = useState<HomepageSectionKey>("hero");
  const [dirtySections, setDirtySections] = useState<Record<HomepageSectionKey, boolean>>({ hero: false, search: false, featured: false, benefits: false, corporate: false, location: false, faqs: false, cta: false });
    ...
    <div className="homepage-cms-grid">
      <div className="min-w-0">
        ...
        <section className="card homepage-cms-panel bg-white p-5 sm:p-7">
          ...
          {activeSection === "hero" && <div className="mt-6 grid gap-5"><FormGroup title="Hero message" description="Edit the headline, supporting line, and primary call to action shown over the homepage image."><CharacterField label="Hero headline" value={homepage.hero_heading} onChange={(value) => update("hero_heading", value, "hero")} limit={CMS_LIMITS.hero_heading} /><CharacterField label="Hero supporting line" value={homepage.hero_subtitle} onChange={(value) => update("hero_subtitle", value, "hero")} limit={CMS_LIMITS.hero_subtitle} /><div className="grid gap-4 sm:grid-cols-2"><CharacterField label="Button label" value={homepage.hero_cta_label} onChange={(value) => update("hero_cta_label", value, "hero")} limit={CMS_LIMITS.button_label} /><CharacterField label="Button link" value={homepage.hero_cta_href} onChange={(value) => update("hero_cta_href", value, "hero")} limit={CMS_LIMITS.hero_cta_href} /></div><CharacterField label="Legacy image caption" value={homepage.hero_image_caption} onChange={(value) => update("hero_image_caption", value, "hero")} limit={CMS_LIMITS.hero_image_caption} help="Kept for existing homepage content; dedicated media captions are edited below." /></FormGroup><HomepageHeroMediaEditor onChange={() => setDirtySections((current) => ({ ...current, hero: true }))} /></div>}
