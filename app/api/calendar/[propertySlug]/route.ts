...
import { normalizeCalendarPropertySlug } from "@/src/lib/calendar/validation";
...
export async function GET(request: Request, { params }: { params: Promise<{ propertySlug: string }> }) {
  ...
  const canonicalSlug = normalizeCalendarPropertySlug(propertySlug);
  const supabase = createSupabaseAdminClient();
  const { data: property } = await supabase.from("properties").select("id, unavailable_dates").eq("slug", canonicalSlug).maybeSingle();
  ...
  return new Response(buildIcsCalendar(events), {
    headers: {
      ...
      "Content-Disposition": `inline; filename="${canonicalSlug}.ics"`,
